...
X_transformed_KS = embedding.fit_transform(distance_matrix)
...
embedding = MDS(n_components=5, dissimilarity="precomputed")
X_transformed_WS = embedding.fit_transform(distance_matrix)


X_final = np.concatenate([X_transformed_KS, X_transformed_WS], axis=1)
...
index_, labels, silhouette = plot_hierarchical_clustering(X_final, \
                    ...
                    distance_matrix=True, criterion="threshold", path=Path(figpath, "clustering_"+str(threshold)+"_"+"WS_and_KS.pdf"))
...
fig2.savefig(Path(figpath,"clustering_"+str(threshold)+"_KS_and_WS_paired_comparison.pdf"))
fig.savefig(Path(figpath,"clustering_"+str(threshold)+"_KS_and_WS_barycenters.pdf"))
